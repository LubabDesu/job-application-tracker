...
export class NotionClient {
    ...
    async createJob(
    ...
    ): Promise<{ jobId: string; notionUrl: string }> {
        const page = (await this.client.pages.create({
            ...
            properties: {
                ...
                [DB_FIELDS.APPLIED_DATE]: {
                    date: { start: new Date().toISOString().substring(0, 10) },
                },
                [DB_FIELDS.JOB_URL]: { url: input.url },
                [DB_FIELDS.SOURCE_PLATFORM]: {
                    select: { name: input.sourcePlatform },
                },
                ...
                ...(input.salaryRange
                    ? {
                          [DB_FIELDS.SALARY_RANGE]: {
                              rich_text: [
                                  { text: { content: input.salaryRange } },
                              ],
                          },
                      }
                    : {}),
    ...
    async enrichJob(
        ...
        await this.client.pages.update({
            ...
            properties: {
                ...
                [DB_FIELDS.SENIORITY]: {
                    select: { name: enrichment.seniority },
                },
        ...
        const remaining = bullets.slice(97);
    ...
    async queryJobs(filters: {
        ...
        if (filters.status)
            andFilters.push({
                property: DB_FIELDS.STATUS,
                select: { equals: filters.status },
            });
        if (filters.jobType)
            andFilters.push({
                property: DB_FIELDS.JOB_TYPE,
                select: { equals: filters.jobType },
            });
        if (filters.company)
            andFilters.push({
                property: DB_FIELDS.COMPANY,
                title: { contains: filters.company },
            });
        const res = await this.client.databases.query({
            database_id: this.dbId,
            filter:
    ...
    async createPrepPage(
        ...
        const page = (await this.client.pages.create({
            ...
            properties: {
                [PREP_DB_FIELDS.NAME]: {
                    title: [
                        { text: { content: `Interview Prep — ${jobTitle}` } },
                    ],
                },
                [PREP_DB_FIELDS.JOB]: {
        ...
        await this.client.pages.update({
            page_id: jobId,
            properties: {
                [DB_FIELDS.PREP_PAGE]: { relation: [{ id: page.id }] },
            },
        });
    ...
    private async appendTextAsBlocks(
        blockId: string,
        text: string,
    ): Promise<void> {
    ...
    private pageToJobRow(page: Record<string, unknown>): JobRow {
        ...
        const getSelect = (p: unknown) =>
            ((p as { select?: { name: string } })?.select?.name ??
                "") as string;
        return {
            ...
            appliedDate:
                (props[DB_FIELDS.APPLIED_DATE] as { date?: { start: string } })
                    ?.date?.start ?? "",
            ...
            enriched:
                (props[DB_FIELDS.ENRICHED] as { checkbox?: boolean })
                    ?.checkbox ?? false,
